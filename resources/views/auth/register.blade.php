...
<form method="POST" action="{{ route('register') }}" class="w-full max-w-sm mx-auto rounded-lg border shadow-md p-5 mb-5">
    ...
    <p for="role-select" class="font-semibold text-gray-700">Profile :</p>
    <div class="flex justify-between items-center">
        <label for="entreprise">Entreprise
            <input type="radio" value="1" id="entreprise" name="role_id">
            <span class="checkmark"></span>
        </label>
        <label for="demandeur">Demandeur
            <input type="radio" value="2" id="demandeur" name="role_id">
            <span class="checkmark"></span>
        </label>
    </div>
    @error('role_id')
